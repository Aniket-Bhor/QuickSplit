export const MemberCard = (member) => `
    <div class="flex items-center justify-between p-3 bg-slate-800/30 rounded-2xl border border-slate-700/30 hover:border-slate-600/50 hover:bg-slate-800/50 transition-all group premium-card card-enter">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 md:w-9 md:h-9 rounded-xl bg-blue-600/10 border border-blue-500/20 flex items-center justify-center text-[10px] md:text-[11px] font-black text-blue-400 uppercase tracking-widest shrink-0">
                ${member.name.charAt(0)}
            </div>
            <span class="text-xs md:text-sm font-bold text-slate-200 truncate max-w-[100px] md:max-w-none">${member.name}</span>
        </div>
        <div class="flex items-center gap-1 md:opacity-0 group-hover:opacity-100 transition-all">
